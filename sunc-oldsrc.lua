local function fankshan()
	...
	pcall(function()
		if assert then
			if ((assert(4 < 3) == nil) or (asset(4 == 3) == "") or (assert(4 == 3) == nil)) then
				print("âš ï¸âš ï¸ YOUR EXECUTOR USES A FAKE ASSERT FUNCTIONâš ï¸âš ï¸\n       This usually means it's used to fake the original UNC test");
				task.wait(0.9);
			end
		end
	end);
	...
	for i, v in pairs(results) do
		if ((i ~= "Drawing.new") and (i ~= "WebSocket.connect") and (i ~= "getgenv_access") and (i ~= "Drawing.Fonts")) then
			if not checkFunctionExists(i) then
				asserted[i] = true;
				cachedFuncs["âŒ " .. i] = "function is nil";
			end
		end
	end
	...
	if not animate then
		warn("âš âš  YOU MAY GET INNACURATE RESULTS, PLEASE JOIN A BASIC ROBLOX PLACE âš âš ");
	end
	EXEname = nil;
	print("Loading...");
	local Soap = tick();
	...
	local a, b = pcall(function()
		if debug.getconstant then
			...
			AsCon(b ~= "Hello, world!", "debug.getconstant", "SKID ALERT: ðŸ˜âŒðŸ’”ðŸ‘");
	...
	local a, b = pcall(function()
		if debug.getconstants then
			...
			AsCon(constants[1] ~= "50000", "debug.getconstants", "âš âš  ATTENTION: BETA MALE DETECTED âš âš ");
	...
	print("â— Starting basic loadstring testing...");
	local animate = game:GetService("Players").LocalPlayer.Character.Animate;
	local a, b = pcall(function()
	...
	if not success then
		AsCon(4 < 3, "loadstring", "loadstring support is broken (ðŸ’€ðŸ’€ðŸ˜­)");
	end
	if (results['loadstring'] == false) then
		AsCon(4 < 3, "loadstring[basic]", "Basic loadstring test failed");
	end
	getgenv().lol = false;
	print("â— Finished basic loadstring testing");
	print("â— Starting simple loadstring URL testing...");
	...
	local function checkSimpleUrl()
		...
		if (response and (results == "CAND_MA_IA_FLAMA")) then
			print("âœ… Passed the simple loadstring test. Short obfuscated scripts can be executed.");
			getgenv().lol = true;
		else
	...
	print("â— Finished simple loadstring URL testing");
	print("â— Starting complicated loadstring URL testing...");
	...
	print("â— Finished complicated loadstring URL testing");
	...
	for _, result in ipairs(sortedResults) do
		if result.passed then
			print("âœ… " .. result.test);
		else
			print("âŒ " .. result.test);
		end
	end
	...
	if (game.PlaceId ~= 4483381587) then
		print("âš  Your results may not be as accurate, I recommend joining 'a literal baseplate' âš ");
	end
	print("\240\159\152\142 Passed the test with " .. SRATE .. "% success rate (" .. passedTests .. " out of " .. totalTests .. ")");
	print("âŒ Total tests failed: " .. (totalTests - passedTests));
